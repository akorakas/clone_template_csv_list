...
locals {
  ...
  parsed_vm_list = [
    for vm in local.raw_vm_list : {
      ...
      ctk_enabled            = length(split(",", vm)) > 10 && split(",", vm)[10] != "" ? lower(split(",", vm)[10]) == "true" : false
    }
  ]
}
...
resource "vsphere_virtual_machine" "vm" {
  ...
  # Extra Config for Change Tracking (ctkEnabled) on all disks
  extra_config = {
    "scsi0:0.ctkEnabled" = tostring(each.value.ctk_enabled)
    "scsi0:1.ctkEnabled" = tostring(each.value.ctk_enabled)
  }
